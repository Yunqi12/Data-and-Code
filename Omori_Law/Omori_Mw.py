...
def find_optimal_a(bin_loc, meta): # Finding the a-value
    ...
    for a in a_values:
        params = [a, 1.07, 5.9, 1.68, 0.004, 0.8335]
...
cat = read_cat('/Users/yunqhuang/Desktop/Code_Seismica/Results/Python/WP_50k_AS_Mw.txt')
...
t_termi = UTC('20300101') # not upper bound
...
print(' Mainshock magnitude: %.3f \n' % ms['mag'],
      'Mainshock time: %s \n'         % ms['obdt'],
      'Minimum magnitude: %.3f \n'    % meta['Mcut'],
      'Maximum radius: %.3f \n'       % meta['rmax'],
      'Start time: %s \n'             % meta['t_start'],
      'End time: %s \n'               % meta['t_termi'],
      '# events selected: %d \n'      % select.sum())
...
a = find_optimal_a(bin_loc, meta);
plt.plot(bin_loc, omorilaw_RJ1989(bin_loc, params = [a, 1.07, 5.9, 1.68, 0.004, 0.8335]), c='lime', label='Reasenberg & Jones (1989) {a,b,c,p}={%.3f, 1.07, 0.004, 0.8335}' % a)

plt.plot(bin_loc, omorilaw_RJ1989(bin_loc, params = [-1.67, 0.91, 5.9, 1.68, 0.05, 1.08]), c='purple', label='Generic SCR aftershock sequences {a,b,c,p}={-1.67,0.91,0.05,1.08}')


plt.legend(loc='lower left',fontsize=14)
plt.title('Mc = 1.68 (Mw)')
plt.show()